...
import { flatten, unflatten } from "sy-framework/lib/widget/table";

//树形数据转换为平级数据，treeData为包含children字段的数组
let flattenData = flatten(treeData);

//平级数据转树形数据，id、parentId为flatData中标识父关系的字段名称
